...
@allure.story("Тест поиска существующего товара Edge_allure")
@allure.title("Тест поиска существующего товара Edge_allure")
def test_search(browser_data:webdriver.Edge, act:ActionChains):
    ...
    # browser_data.save_screenshot('/Diplom/Edge_allure/search.png')
    ...
    with allure.step('Делаем скриншот'):
        allure.attach(browser_data.get_screenshot_as_png(), name='search1',
                      attachment_type=AttachmentType.PNG)
    # browser_data.save_screenshot('/Diplom/Edge_allure/search1.png')
    with allure.step('Нажатие категорий'):
        ...
        for element_name in name_menu:
            ...
            try:
                ...
                # browser_data.save_screenshot('/Diplom/Edge_allure/search2.png')
